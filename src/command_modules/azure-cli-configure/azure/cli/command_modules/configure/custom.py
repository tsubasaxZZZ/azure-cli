...
def _handle_global_configuration(config):
    ...
    #global_config = config.config_parser
    #global_config.read(config.config_path)
...
def handle_configure(cli_ctx, defaults=None):
    if defaults:
        ...
        for default in defaults:
            ...
            cli_ctx.config.set_value(DEFAULTS_SECTION, parts[0], _normalize_config_value(parts[1]))
        return

    # if nothing supplied, we go interactively
    try:
        print(MSG_INTRO)
        _handle_global_configuration(cli_ctx.config)
        print(MSG_CLOSING)
